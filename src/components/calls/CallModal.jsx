...
const CallModal = ({ isOpen, call, isIncoming, onAccept, onDecline, onEnd }) => {
  ...
  const title = isIncoming ? (call.type === 'video' ? 'Входящий видеозвонок' : 'Входящий звонок') : (call.type === 'video' ? 'Исходящий видеозвонок' : 'Исходящий звонок');

  return (
    <div className={styles.overlay}>
      <div className={styles.modal}>
        <div className={styles.avatarWrap}>
          {user?.avatar ? <img src={user.avatar} alt={user?.displayName || user?.username} /> : <div className={styles.avatarPlaceholder}>{(user?.displayName || user?.username || '?').slice(0,1).toUpperCase()}</div>}
        </div>
